...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo ADDON_PUBLIC_PATH;?>/LCalendar.css">
		...
		<script type="text/javascript" src="<?php echo ADDON_PUBLIC_PATH;?>/LCalendar.js"></script>
		<script type="text/javascript" src="<?php echo ADDON_PUBLIC_PATH;?>/bootstrap.min.js"></script>
		<style>
			...
			.lis div i{background: url(<?php echo ADDON_PUBLIC_PATH;?>/item_sel.gif) no-repeat;display: none; bottom: 0;height: 12px;overflow: hidden;position: absolute;right: 0;text-indent: -9999em;width: 12px;}
					...
					<div class="col-sm-10 clear">
						...
						<?php  for($i = 1;$i < 41;$i++){ ?>
							<option value="<?php echo $i; ?>">
								<?php  if($i == 36){ echo $i."(C5)"; }else if($i == 34 || $i == 35){ echo $i."(C2)"; }else{ echo $i."(C1)"; } ?>
							</option>
						<?php	 } ?>
						</select>
					</div>
					...
					// $(document).ready(function () {
						function dateChange(){							
							var ch = $("#ch").val();
					    		...
					    		$.ajax({
						            type:"post",
						            url:"<?php echo addons_url('Forms://Wap/Paiban');?>",
...
//										   })
		...
		<script type="text/javascript">
			function checkForm(){
				...
				$.ajax({
		            type:"post",
		            url:"<?php echo addons_url('Forms://Wap/xuanze');?>",
		            data:{id:ids},
		            dataType:"json",
		            success:function(result){
