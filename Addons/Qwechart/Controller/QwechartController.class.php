...
namespace Addons\Qwechart\Controller;

use Addons\Qwechart\Controller\BaseController;

class QwechartController extends \Addons\Qwechart\Controller\BaseController{
...
// 		print_r($tree2);die;
		$con = count($tree2['button']);
		if ($con>0){
			$tree = '{  
	             "button":[ ';
			foreach ($tree2['button'] as $key => $val){
				if ($key == ($con-1)){
					if ($val['sub_button']){
						$tree.='{
									"name":"'.$val['name'].'",
