(function($) {
    $.selectSuggest = function(target, data, itemSelectFunction) {
        ...
        var suggestContainerLeft = $('#' + target).offset().left-8;
...
//      	var id=$("#date1").val();
        	...
        	$("#date2").val(this.id);
//      	alert($("#date2").val())
            $('#' + target).val(this.innerText).trigger("change");
        ...
        var _initItems = function(items) {
            ...
            for (var i = 0; i < items.length; i++) {
            		if(items[i].jxname.length > maxFontNumber){
            			maxFontNumber = items[i].jxname.length;
            			}
                var suggestItem = $('<div></div>'); //创建一个子<div>
                //alert(items[i].id)
                suggestItem.attr('id', items[i].id);
                var id=items[i].id;  
                suggestItem.append(items[i].jxname);
                suggestItem.css({
        ...
        var inputChange = function() {
            ...
            return $.grep(data,
            function(value) {
                return matcher.test(value.jxname);
            });
        }
