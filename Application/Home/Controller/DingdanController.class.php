...
class DingdanController extends HomeController {
	...
	public function ding_list(){
		...
		->join('wp_xueyuan x on d.xid=x.id')
		->join('wp_paiban p on d.pid=p.id')
		->join('LEFT join wp_jiaxiao jx on d.jxid = jx.id')
		->field('*,d.id as id,d.status as status,jx.jxname')
		->where($where)
	...
	public function xiao_list(){
		...
		->join('wp_xueyuan x on d.xid=x.id')
		->join('wp_paiban p on d.pid=p.id')
		->field('*,d.id as id,d.status as status,d.fs')
		->where($where)
		->limit($Page->firstRow.','.$Page->listRows)
		->order('d.id desc')
		->select();	
		// var_dump($where,$xiao_list);die;
		// $request = ( array ) I ( 'request.' );
	...
	public function refund(){
		...
		if($fs == 2){
			...
			$where['d.fs'] = $fs;
			$list = $dingdan->table('wp_dingdan d')
				->join('wp_jiaxiao jx on d.jxid = jx.id')
				->field('*,d.id as id')
				->where($where)
				->find();
			$op_user_id = $list['mch_id'];
		}else{
			$where['d.id'] = $id;
			$list = $dingdan->table('wp_dingdan d')
...
/* 					case 4:
  ...
  //   	}
   ...
   //  		if($datetimepicker == $datetimepicker1){
		...
		$Page = new \Think\Page ( $totalCount,10 );
		$ck_list = $dingdan->field("count(*) as count,pdata,daddtime,DATE_FORMAT(daddtime,'%Y-%m-%d') as dadd,sum(money) as subtotal,fs")
					->where($where)
					->group("dadd")
					->limit($Page->firstRow.','.$Page->listRows)
					->order('daddtime desc')
		...
		foreach ($cunkuan_list as $key => $value) {
			...
			//$data11=M('tuikuan')->field("sum(money) as subtotal_tui")->where($condtion)->select();
			...
			//手续费金额
			if($cunkuan_list[$key]['dadd'] == '2017-11-15'){
				$cunkuan_list[$key]['subtotal_sx'] = ($cunkuan_list[$key]['subtotal'] - $cunkuan_list[$key]['subtotal_tui']) * 0.006;
				$cunkuan_list[$key]['subtotal_sx'] = sprintf("%.2f", $cunkuan_list[$key]['subtotal_sx']) + 0.03;
			}else{
				$cunkuan_list[$key]['subtotal_sx'] = ($cunkuan_list[$key]['subtotal'] - $cunkuan_list[$key]['subtotal_tui']) * 0.006;
