...
class DayinController extends HomeController {
	...
	public function dayin_list(){
		...
		if($id){
			...
			if($status == 1){
				$result['url'] = "http://".$_SERVER['HTTP_HOST']."/index.php?s=/Home/Dayin/dayin_list/";
			}else{
				$result['status'] = 0;
	...
	//根据教练身份证搜索驾校预约订单信息
	public function chaxun_jxdingdan(){
		header("Content-Type: text/html;charset=utf-8"); 
		$jl_card = I('jl_card');
		$dingdan = M('dingdan');
		$where['jl.jcard'] = $jl_card;
		$where['d.status'] = 0;
		$where['d.fs'] = 2;
		$list = $dingdan->table('wp_dingdan d')
				//->join('wp_xueyuan x on d.xid = x.id')
				->join('wp_jiaolian jl on jl.id = d.jid')
				//->join('wp_jiaxiao jx on x.jx = jx.id')
				->field('*,d.id as id')
				->where($where)
				->select();
		//dump($dingdan->getlastsql());die;
		if($list){
	...
	public function huoqu(){
		...
		if($get_date < $today_date){
			...
			foreach($list as $k=>$v){
				$dangqian_date = date('Y-m-d');
				$dagnqian_hour = date('H',time());
				if(strtotime($v['data']) == strtotime($dangqian_date)){
					$yue_hour = substr($v['time'],0,2);
					if($yue_hour <= $dagnqian_hour){
						$list[$k]['status'] = 1;
					}
				}
			}
