...
class JiaxiaoController extends HomeController {
	function _initialize() {
		...
		//驾校详情
		// $res ['title'] = '驾校详情';
		// $res ['url'] = U ( 'jiaolian_xq', $param );
		// $res ['class'] = strpos ( $act, 'jiaxiao_xq' ) !== false ? 'current' : '';
		// $nav [] = $res;
	...
	public function jiaxiao_list(){
		...
		$totalCount = $jiaxiao->table('wp_jiaxiao jx')
	...
	public function jiaxiao_xq(){
		//dump($_GET);die;
		$jiaolian= M('jiaolian');
		$id = I('id');
		$mdm = I('mdm');

		$data=$jiaolian->table('wp_jiaolian jl')
		                ->join('wp_jiaxiao jx on jl.jxid = jx.id')
						->field('*,jl.id as id,count(*) as count')
		                ->where(array('jxid'=>$id))
		                ->select();
		
		$totalCount =  intval($data[0]['count']);
