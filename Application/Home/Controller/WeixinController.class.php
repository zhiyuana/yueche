...
class WeixinController extends HomeController {
	...
	public function index() {
		...
		$data ['ToUserName'] == $this->init_follow ( $data );
// 		$weixin->replyText("你好：".$data['Event']."——".$data ['EventKey']."——".$data['Content'].'——'.$data ['FromUserName'].'——'.$data ['ToUserName']);
		if ($data['Event'] != 'LOCATION'){
// 			$weixin->replyText("你好：".$data ['EventKey'].$data['Content'].'——'.$data ['FromUserName'].'——'.$data ['ToUserName']);
			// 回复数据
