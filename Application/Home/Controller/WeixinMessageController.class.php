...
// 		$list ['list_data'] = M ()->query ( $sql );
		...
		$ids = M('weixin_message')->where($map)->field('MAX(id) as mid')->order('id desc')->group('FromUserName')->page($page,$row)->select();

		/* 查询记录总数 */
		//$count = M ()->query ( "SELECT count(DISTINCT FromUserName) as num FROM `{$px}weixin_message`" );
		$count = M ()->query ( "SELECT count(DISTINCT FromUserName) as num FROM `{$px}weixin_message` WHERE `type` = 0 AND `ToUserName` = '".$token."'" );
		...
		if (!empty($arr)){
		    ...
		    $list ['list_data'] = M('weixin_message')->where($map1)->order('id DESC')->page($page,$row)->select();
		}
