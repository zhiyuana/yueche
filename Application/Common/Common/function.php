 ...
 * $url:图片的地址
 */
function readIdCard($image_file){
	$content = base64_encode(file_get_contents($image_file));
	$host = "https://dm-51.data.aliyun.com";
	$path = "/rest/160601/ocr/ocr_idcard.json";
	$method = "POST";
	//$appcode = "8e85f3b05a0848e2be114473a3eab7b6";
	$appcode = "5b122a984bb44b0f977e865cbfe278bd";
	$headers = array();
	...
	$bodys = "{\"inputs\":[{\"image\":{\"dataType\":50,\"dataValue\":\"".$content."\"},\"configure\":{\"dataType\":50,\"dataValue\":\"{\\\"side\\\":\\\"face\\\"}\"}}]}";
	...
	$arr = $arr['outputs'][0]['outputValue']['dataValue'];
	return $arr;
}
/**
 * 获取状态值对应的标题
...
function get_access_token($token = '', $update = false) {
	...
	$appid = 'wxd07982249e620b62';
	$appsecret = 'f712d4313050708b641c51ceefb80f83';
	$access_token = get_access_token_by_apppid($appid , $appsecret);
...
function getOut_trade_no($out_trade_no = ""){
	$out_trade_no = date('Ymd') . time() . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
	$weizhifu = M('weizhifu');
	$where['out_trade_no'] = $out_trade_no;
	$list = $weizhifu->where("out_trade_no = ".$out_trade_no)->find();
	if($list){
		getOut_trade_no();die;
	}else{
		return $out_trade_no;
