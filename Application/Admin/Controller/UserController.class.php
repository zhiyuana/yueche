...
class UserController extends AdminController {
	...
	function edit(){
		...
		$user = M('user');
		$where['uid'] = $id;
		$user_detail = $user->table('wp_user u')
			->join('LEFT JOIN wp_jiaxiao jx on u.jxid = jx.id')
			->field('uid,nickname,jxid,jx.jxname')
			->where($where)
			->find();
		$user_detail['nickname'] = trim($user_detail['nickname'],'\"');
		$jiaxiao = M('jiaxiao');
		...
		if(IS_POST){
			// $detail['uid'] = $_POST['uid'];
			// $detail['jiaxiao'] = $_POST['jiaxiao'];
			$map['uid'] = $_POST['uid'];
			$jxid = $_POST['jxid'];
			$jiaxiaoid = $_POST['jiaxiaoid'];
			if(empty($jxid)){
				$update_info['jxid'] = $jiaxiaoid;
			}else{
				$update_info['jxid'] = $jxid;
			}
			// file_put_contents('./wangdi.txt',$detail);die;
			$user = M('user');
			$sql = "update wp_user set jxid=".$update_info['jxid']." where uid=".$map['uid'];
